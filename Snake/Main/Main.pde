//need to include the pause menu
//just resume and quit- press start button to pause
...
void draw(){
 ...
 if(gamePause){
   ...
   textAlign(CENTER);
   text("PAUSE",375,375);
   text("PRESS PLAYER 1 START TO CONTINUE",375,420);
   text("PRESS PLAYER 2 START TO QUIT",375,465);
 }
 if(!gameOver){
   firstOver = true;
  if(gameStart){
   ...
   textAlign(CENTER);
   text("SNAKE",375,375);
   text("PRESS PLAYER 1 START",375,420);
   textAlign(RIGHT);
   textSize(12);
  ...
  for(int i = 0; i < 25; i++){
   ...
   text("PRESS PLAYER 1 START TO RESTART",375,420);
   text("PRESS PLAYER 2 START TO QUIT",375,465);
   if(code){
     player.close();
     player2.play();
...
void keyPressed(){
  ...
  else if(key == Buttons.PLAYER_1_JOYSTICK_DOWN){
    if(!gameStart)snake.setDirection("Down");
      if(codeint == 2 || codeint == 3){
        konami[codeint] = 1;
      }
  }
  else if(key == Buttons.PLAYER_1_JOYSTICK_LEFT){
    if(!gameStart)snake.setDirection("Left");
      if(codeint == 4 || codeint == 6){
        konami[codeint] = 1;
      }
  }
  else if(key == Buttons.PLAYER_1_JOYSTICK_UP){
  ...
  else if(key == Buttons.PLAYER_1_START){
    if(gameOver){
      score = 0;
    }
    if(codeint == 10){
  ...
  else if(key == Buttons.PLAYER_2_START){
    if(gameStart || gamePause || gameOver){
      exit();
    }
  }
  else if(key == Buttons.PLAYER_1_BUTTON_1){
      if(codeint == 9){
        konami[codeint] = 1;
      }
  }
  else if(key == Buttons.PLAYER_1_BUTTON_2){
      if(codeint == 8){
        konami[codeint] = 1;
      }
  }
